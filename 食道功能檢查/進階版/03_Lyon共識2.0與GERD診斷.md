...
```mermaid
flowchart TD
    A[疑似 GERD 患者] --> B{已做過胃鏡？}

    B -->|是| C{內視鏡發現？}
    B -->|否| B1[先進行胃鏡評估]
    B1 --> C

    C -->|LA C/D 食道炎<br>長段 Barrett's<br>消化性狹窄| D[確定性 GERD<br>開始治療]
    C -->|LA A/B 食道炎| E[不確定<br>建議停藥後重複胃鏡<br>或進行逆流監測]
    C -->|正常或非特異性發現| F{進行逆流監測<br>停藥狀態}

    F --> G{AET 結果？}

    G -->|AET > 6%| H[確定性 GERD]
    G -->|AET < 4%<br>且逆流 <40 次| I[排除病理性 GERD<br>考慮功能性診斷]
    G -->|AET 4-6%<br>或逆流 ≥40 但 AET <4%| J[不確定區間]

    J --> K{輔助指標評估}
    K -->|BI 降低 +<br>PSPWI 降低 +<br>組織病理異常| L[傾向 GERD]
    K -->|BI 正常 +<br>PSPWI 正常 +<br>組織正常| M[傾向非 GERD<br>考慮功能性診斷]
    K -->|結果不一致| N[綜合臨床判斷<br>可考慮延長監測或重複檢查]

    D --> O{PPI 治療反應？}
    O -->|症狀改善| P[繼續治療]
    O -->|症狀持續| Q[服藥狀態下<br>pH-阻抗監測]
```
